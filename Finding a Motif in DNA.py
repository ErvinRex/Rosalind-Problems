...
def motif_find(s, t):
    poslist = []
    pos=0
    while pos<=len(s) - len(t): #as long as pos is not greater than or equal to the length of s - length of t
        if s[pos:pos + len(t)] == t: #if at this character in the sequence, s is identical to t (reading in the length of t)
            poslist.append(pos + 1) # add 1 to adjust for 0-based indexing
            pos += 1 # add 1 to the position to read from the next base
        else:
            pos += 1 # if it is not, don't continue making list, just read from the next sequence and loop again
    print(poslist)
